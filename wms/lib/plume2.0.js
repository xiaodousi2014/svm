...
(function (root, $) {
    ...
    $.extend({
         ...
         */
        loadData: function (name, flag, fun, load) {
            ...
            if (flag.indexOf("nochangeurl") === -1) {
                try {
                    !push && window.history.pushState({}, '', name + prams);
                } catch (e) {
    ...
    root.Plume = {
         ...
         * @returns {*}
         */
        defaultSet: function (obj) {
            if (utils.isJSON(obj)) {
                this.setParam = $.extend({}, this.setParam, obj);
            }
        },
                ...
                loadHtml: function (url, config, fun, cache) {
                    ...
                    try {
                        if (utils.support.storage && cache) {
                            ...
                            $('' + config.container).load(url + ' ' + config.load, function () {
                                createHtml(this, fun, null);
                            });
                        }
                    } catch (e) {
